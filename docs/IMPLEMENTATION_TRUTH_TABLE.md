...
| Session-start graph wake-up audit | Implemented | session start now runs a bounded two-phase audit: legacy knowledge normalization persists missing information entities/typed edges, and behavior taxonomy audits turn-attached Adam/feedback bundles to strengthen first-order behavior memes and materialize bounded behavior memodes; session metadata now records `session_graph_normalization`, `session_graph_taxonomy`, and `session_graph_wakeup`, while traces emit `GRAPH_NORMALIZATION`, `GRAPH_TAXONOMY_AUDIT`, and `GRAPH_WAKEUP_AUDIT`; the Adam-identity MLX review hooks are code/test-covered, while live-model quality remains bounded by the current prompt/schema |
